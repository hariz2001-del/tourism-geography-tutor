-- Private, atomic remediation path for CH4's unapproved DeepSeek draft bank.
-- It cannot replace another chapter or touch human-generated rows. Every
-- source/citation is validated before deletion, so failed input rolls back.

create or replace function public.replace_ch4_deepseek_draft_bank(p_records jsonb)
returns jsonb
language plpgsql
security definer
...
begin
  if jsonb_typeof(p_records) <> 'array' or jsonb_array_length(p_records) = 0 then
    raise exception 'CH4 replacement requires a non-empty JSON array';
  end if;

  for record in select value from jsonb_array_elements(p_records)
  loop
    if record->>'chapter_code' <> 'CH4'
      ...
      raise exception 'Replacement records must be CH4 draft/deepseek_draft MCQ or subjective items';
    end if;
    ...
    if source_id is null then
      raise exception 'No exact published CH4 source/citation for %', record->>'source_title';
    end if;
  end loop;
...
revoke all on function public.replace_ch4_deepseek_draft_bank(jsonb) from public, anon, authenticated;
grant execute on function public.replace_ch4_deepseek_draft_bank(jsonb) to service_role;
