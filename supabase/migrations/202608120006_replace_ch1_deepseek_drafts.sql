-- Private, atomic replacement path for CH1's unapproved DeepSeek draft bank.
-- It is chapter-bound and validates every source/citation before deleting rows.

create or replace function public.replace_ch1_deepseek_draft_bank(p_records jsonb)
returns jsonb
language plpgsql
security definer
...
begin
  if jsonb_typeof(p_records) <> 'array' or jsonb_array_length(p_records) = 0 then
    raise exception 'CH1 replacement requires a non-empty JSON array';
  end if;

  for record in select value from jsonb_array_elements(p_records)
  loop
    if record->>'chapter_code' <> 'CH1'
      ...
      raise exception 'Replacement records must be CH1 draft/deepseek_draft MCQ or subjective items';
    end if;
    ...
    if source_id is null then
      raise exception 'No exact published CH1 source/citation for %', record->>'source_title';
    end if;
  end loop;
...
revoke all on function public.replace_ch1_deepseek_draft_bank(jsonb) from public, anon, authenticated;
grant execute on function public.replace_ch1_deepseek_draft_bank(jsonb) to service_role;
