    ...
    return <main className="mx-auto min-h-screen max-w-4xl space-y-8 px-6 py-10">
      ...
      <div className="space-y-6">{questions.map((question, index) => {
        ...
        return <section key={question.id} className="space-y-4 rounded-card border border-graticule bg-surface p-5">
          ...
          {isMcqResult(result) ? <p className="text-ink">{result.isCorrect ? "Correct." : "Not quite."} {result.explanation}</p> : <div className="space-y-3"><p className="text-ink">Your written answer has been marked against the approved criteria.</p>{result.criteria.map((criterion, criterionIndex) => <div key={criterionIndex} className="rounded-card border border-graticule p-3"><p className="font-medium text-ink-strong">{criterion.awardedMarks} / {criterion.maxMarks} marks</p><p className="mt-1 text-ink">{criterion.feedback}</p>{criterion.citations.map((citation, citationIndex) => <div className="mt-2" key={citationIndex}><CitationCard citation={citation} /></div>)}</div>)}</div>}
          <CitationCard citation={question.citation} />
        </section>;
      })}</div>
    </main>;
  }
