...
describe("POST /api/quiz/answer", () => {
  it("returns only feedback after server-side answer checking", async () => {
    const POST = createQuizAnswerRouteHandler(() => ({
      checkApprovedQuizAnswer: vi.fn().mockResolvedValue({ isCorrect: true, explanation: "Reviewed explanation." }),
    }));
    ...
    expect(await response.json()).toEqual({ data: { isCorrect: true, explanation: "Reviewed explanation." } });
  });
