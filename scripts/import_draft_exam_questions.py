...
def replace_ch1_rest(records: list[dict[str, Any]], client: Any) -> dict[str, int]:
    """Atomically replace only CH1 deepseek draft rows through a private RPC."""
    validate(records)
    if not records or any(record["chapter_code"] != "CH1" for record in records):
        raise ValueError("The CH1 replacement path accepts CH1 records only.")
    result = client.request("POST", "rpc/replace_ch1_deepseek_draft_bank", payload={"p_records": records})
    if not isinstance(result, dict) or not all(isinstance(result.get(key), int) for key in ("deleted", "inserted")):
        raise RuntimeError("CH1 replacement RPC returned an invalid result.")
    return {"deleted": result["deleted"], "inserted": result["inserted"]}
...
def main() -> int:
    ...
    parser.add_argument("--replace-ch1-drafts", action="store_true",
                        help="Atomically replace only existing CH1 draft/deepseek_draft rows.")
    ...
    validate(records)
    replacement_flags = (args.replace_ch1_drafts, args.replace_ch2_drafts, args.replace_ch4_drafts)
    if any(replacement_flags) and not args.apply:
        raise RuntimeError("A draft replacement flag requires --apply.")
    if sum(replacement_flags) > 1:
    ...
    if args.replace_ch1_drafts:
        if database_url:
            raise RuntimeError("CH1 replacement uses the private Supabase RPC; unset COURSE_BRAIN_DATABASE_URL.")
        supabase_url = os.environ.get("SUPABASE_URL")
        service_role_key = os.environ.get("SUPABASE_SERVICE_ROLE_KEY")
        if not supabase_url or not service_role_key:
            raise RuntimeError("CH1 replacement requires SUPABASE_URL plus SUPABASE_SERVICE_ROLE_KEY.")
        result = replace_ch1_rest(records, SupabaseRestClient(supabase_url, service_role_key))
        print(f"Replaced {result['deleted']} CH1 draft question(s) with {result['inserted']} revised draft question(s).")
        return 0
